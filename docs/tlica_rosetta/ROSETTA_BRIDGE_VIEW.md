...
| B-ORP-007 | Action-calibrated projected PCE | Deterministic projected profile plus future MSI assignment plus universal-domain ranking | `FutureMSIModel`, `GlobalPreservationRanking`, `ProjectedPCE` | machine_verified definitions/theorems; primitive calibration fields |
| B-ORP-008 | Conditional action differentiation | Strict projected-PCE difference follows from strict global-rank difference; no foundation-level existence claim | `ProjectedPCE.strictly_differentiates_of_rank_lt` | machine_verified conditional theorem |
| B-ORP-009 | Stochastic projected PCE | Probability-valued or expected projected future profile/rank model | none | application_deferred |
