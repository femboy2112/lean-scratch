...
| AGY-005 | Finite feasible maximizer existence | Finset-enumerated finite feasible-action sufficient condition | `exists_selectsFeasibleAction_of_finset` | `machine_verified_theorem` | agency layer |
...
| AFF-002 | Branch-profile shift | Distance and equality-based branch profile shift, branch union-distance bounds, plus future-content/profile bridge | `branchUnionDistance`, `branchUnionDistance_nonneg`, `branchUnionDistance_le_one`, `branchSharedDistance`, `branchProfileShift`, `branchFutureContents_eq_of_branchProfile_eq`, `branchProfile_ne_of_branchFutureContents_ne` | `machine_verified_definition`; `machine_verified_theorem` | differentiated-affect layer |
...
| PRF-005 | Shell thresholds | Ordered thresholds, interior half-open shell-membership predicate, and interior index-distance helper | `ShellThresholds`, `shellOf`, `interiorShellIndexDistance`, `shellOf_nonneg`, `shellOf_le_one` | `machine_verified_definition`; `machine_verified_theorem`; `primitive_structure_field` | `04_profiles.md` §4.3.1 |
| PRF-006 | Same/interior-shell bounds | Bound on distance for profiles in the same shell, conservative cross-interior-shell bound, and profile-level pointwise shared-distance bound | `sameShellBound`, `sameShellBound_of_shellOf`, `sameInteriorShell_distance_bound`, `interiorShell_pair_bound`, `shellStableDistanceBound_of_pointwise`, `shellStableDistanceVanishing_simple` | `machine_verified_theorem` | `profile_comparison_v0_2.md` §5.3 |
| PRF-007 | General shell-stratified bound | Full seven-shell bound by maximum shell gap and shell distance | `shellStratifiedBound_deferred` | `deferred_marker_not_theorem` | `profile_comparison_v0_2.md` §5.3; blocked pending explicit boundary-shell conventions |
