...
| TMP-002 | Deterministic generated trajectory | Predicate verifying that a supplied trajectory follows deterministic projection, with named generated-step and future-content unfold helpers | `generatedBy`, `generatedBy_step`, `generatedBy_step_projectedProfile`, `trajectoryFutureContents`, `trajectoryFutureContents_eq_branchFutureContents`, `oneStepBranch` | `machine_verified_definition`; `machine_verified_theorem` | temporal-trajectory layer |
...
| PRF-005 | Shell thresholds | Ordered thresholds, interior and seven-shell boundary-aware shell predicates, endpoint functions, and shell membership bounds | `ShellThresholds`, `shellOf`, `interiorShellIndexDistance`, `ShellIndex7`, `shellLowerEndpoint`, `shellUpperEndpoint`, `shellOf7`, `shellOf_nonneg`, `shellOf_le_one`, `shellOf7_*` | `machine_verified_definition`; `machine_verified_theorem`; `primitive_structure_field` | `04_profiles.md` §4.3.1 |
...
| PRF-007 | General shell-stratified bound | Full seven-shell pointwise bound by endpoint maximum under source-fixed shell boundary conventions | `fullShellStratifiedBound`; legacy `shellStratifiedBound_deferred` superseded | `machine_verified_theorem`; `deprecated_or_superseded` | `profile_comparison_v0_2.md` §5.3; `04_profiles.md` §4.3.1 |
