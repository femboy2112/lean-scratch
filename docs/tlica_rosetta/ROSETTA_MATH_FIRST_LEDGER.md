...
| ORP-003 | Future-state projection | Deterministic transition/projection map parameterized by arbitrary action type `Act`; old singleton action retained as degenerate default | `Action`, `DefaultAction`, `Action.noAction`, `ProjectMap`, `ProjectMap.noAction`, `ProjectMap.project`, `ProjectMap.noAction_projects`, `DefaultProjectMap` | machine_verified definition / primitive structure-field assumption | `orphan_cluster_v0_1.md` §5; v0.4 refinement |
...
| ORP-008A | Optional profile-isomorphism coherence | Extensional domain/value coherence between profiles and stronger future-MSI model projecting to `FutureMSIModel` | `ProfileIso`, `ProfileIso.refl`, `ProfileIso.symm`, `ProfileIso.trans`, `CoherentFutureMSIModel`, `CoherentFutureMSIModel.toFutureMSIModel` | machine_verified structures/definitions; existing `domain_match` remains default weak condition | v0.4 foundation refinement |
...
| ORP-011 | Action-calibrated projected PCE interface | Projected PCE over parameterized foundation `ProjectMap α Act` | `ProjectedPCE`, `ProjectedPCE.nonneg`, `ProjectedPCE.eq_of_future_contents_eq`, `ProjectedPCE.ge_of_rank_ge`, `ProjectedPCE.monotone_of_future_contents_subset`, `ProjectedPCE.strictly_differentiates_of_rank_lt` | machine_verified definition/theorems; arbitrary `Act` supported | action-projection refinement; v0.4 refinement |
...
| AGY-002 | Agency context | Feasible action model with generalized projected-PCE calibration, using reusable project-map-indexed feasibility | `FeasibilityModel`, `FeasibilityModel.noAction_feasible`, `AgencyContext`, `AgencyContext.feasibility`, `AgencyContext.feasible`, `AgencyContext.noAction_feasible` | machine_verified definitions / primitive structure-field assumption; no global agency existence claim | agency layer; v0.4 refinement |
