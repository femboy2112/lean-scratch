...
| AGY-001 to AGY-004 | Agency witness layer | has multiple Lean declarations | `FeasibilityModel`, `AgencyContext`, `selectsFeasibleAction`, `AgencyWitness`, live/PCE-differentiated predicates | First structural witness layer, no global agency existence. |
| AGY-005 | Finite feasible maximizer existence | deferred marker only | `finiteFeasibleSelection_deferred` | Not a theorem. |
| FWL-001 to FWL-004 | Free-will witness layer | has multiple Lean declarations | branch definitions, `FreeWillWitness`, `PCEFreeWillWitness`, collapse theorems | First structural branch-sensitive witness layer. |
| TMP-001 to TMP-004 | Deterministic temporal trajectory substrate | has multiple Lean declarations | `ProfileTrajectory`, `ActionSchedule`, `generatedBy`, stability/divergence declarations | Supplied/verified trajectories only; no global trajectory existence. |
...
| AFF-001 to AFF-006 | Profile/PCE affect kernel | has multiple Lean declarations | PCE valence, branch shift, affect witness, bridge, collapse, intensity declarations | Generic kernel only, not named affect taxonomy. |
