...
| PRF-008 | Pseudo-emetric profile space | has multiple Lean declarations | `ProfileSpace`, `edist_def`, `edist_eq_dInfUnion`, `edist_self_profile`, `edist_symm_profile`, `edist_triangle_profile`, `edist_le_one_profile`, `mem_emetric_ball_iff_dInfUnion_lt` | Profile-comparison convenience layer; `edist` is definitionally the union-form distance, with no change to the underlying distance definitions. |
...
| TMP-001 to TMP-004 | Deterministic temporal trajectory substrate | has multiple Lean declarations | `ProfileTrajectory`, `ActionSchedule`, direct `generatedBy`, generated-step helpers, future-content bridges, stability/divergence declarations, generated adjacent-distance rewrites, shared-distance guarded facts, and one-step branch-distance bounds/metric-style helpers | Supplied/verified trajectories only; no global trajectory existence. |
