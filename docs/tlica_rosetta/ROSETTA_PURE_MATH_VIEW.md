...
The scheduled branch future contents at time `n` are the existing free-will branch future contents applied to the profile at `n` and the scheduled action at `n`. Adjacent-time stability is measured through the existing profile-comparison distances: union-domain distance `dInfUnion` and shared-domain distance `dInfShared`. Generated-trajectory helper theorems rewrite adjacent union/shared distances to the scheduled projected profile or one-step branch. Shared-distance unit bounds remain guarded by nonempty shared domain because the empty-shared-domain convention returns top. Uniform and eventual union-step stability are predicates over these adjacent-time distances.
...
Lean carriers: `ProfileTrajectory`, `ActionSchedule`, `generatedBy`, `generatedBy_step*`, `trajectoryFutureContents`, `stepUnionDistance`, `generatedBy_stepUnionDistance_*`, `stepSharedDistance`, `stepSharedDistance_*`, `generatedBy_stepSharedDistance_*`, `unionStepStable`, `eventuallyUnionStepStable`, `DivergentTrajectories`, `oneStepBranch`, `oneStepBranch_eq_*`, `oneStepBranchUnionDistance`, `oneStepBranchUnionDistance_*`, `freeWillWitness_live_branch_contents_distinct`, `freeWillWitness_oneStep_branch_contents_distinct`, `pceFreeWillWitness_oneStep_branch_contents_distinct`, `stochasticTrajectory_deferred`, `phenomenologicalDuration_deferred`, `temporalContinuityMetric_deferred`.
