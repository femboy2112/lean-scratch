...
Lean v0.4.1 represents this feasibility component with `FeasibilityModel α Act proj`: for each current profile it supplies a set of feasible actions, and it requires the distinguished no-action element of the projection map to be feasible. Feasible sets remain supplied and calibrated by the application context; the formal layer does not assert that a projected-PCE maximizer exists globally.
